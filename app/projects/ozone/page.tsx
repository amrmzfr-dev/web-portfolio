...
import { OzoneHeroDiagram } from './OzoneHeroDiagram'
        ...
        <div className="proj-content">
          <div className="proj-main">
            ...
            <div className="content-block">
              ...
              <p className="content-text" style={{ marginTop: '1rem' }}>Second: timestamp accuracy. The previous system timestamped sessions at server receive time. Machines in workshops lose WiFi regularly: a power cut, a router reboot, or a dead spot. When connectivity restored, the server received all buffered sessions simultaneously, stamping every one with the reconnection time. A week&apos;s data would appear as a single day. I solved this by having the ESP32 timestamp each session at the moment it occurs and persist that timestamp to flash before any network attempt. When the device reconnects after days offline, each session carries the time it actually happened. The <code>ClockSource</code> enum captures the confidence of every timestamp: NTP synced at the top, then hardware RTC validated, then NVS anchored fallback when the RTC has lost power, then millis only as a last resort. The backend reads this field on every record and the analytics layer automatically quarantines low confidence sources from billing reports, so bad data is surfaced explicitly rather than silently accepted.</p>
              <div className="proj-dashboard-capture">
                <div className="photo-frame">
                  <Image
                    src="/images/ozone/ozonetrack-dashboard.png"
                    alt="OzoneTrack outlet dashboard with daily treatments chart aligned to real session dates"
                    fill
                ...
                <p className="gallery-caption">Outlet reporting: daily charts reflect when sessions actually occurred, not when the server received them.</p>
              </div>
            </div>
          ...
          <div className="proj-sidebar">
            ...
            <div className="sidebar-block">
              ...
              <div className="sidebar-val">Clock integrity architecture with timestamps persisted to flash before any network attempt. ESP32 gated session starts so phantom counts cannot occur structurally. Live billing in production.</div>
            </div>
          </div>
        </div>
